        ...
        (Dish.build(
            recipes = mutableListOf<Recipe>(
                Recipe.build(
                    "Утка по Татарски",
                    mutableListOf<CookingStep>(
                        CookingStep("Помыть", "Поместите утку в ванну и тщательно промойте ее под душем", 10),
                        CookingStep("Мариновать", "Обработайте утку специями и соусами по вкусу и оставьте ее в таком состоянии на некоторое время для более яркого вкуса", 60),
                        CookingStep("Запечь", "Предварительно разогрейте духовку до 451 градуса по Френгейту и поместите туда утку на протвени, ожидайте", 60),
                        CookingStep("Разрезать", "После полной готовности, достаньте утку из духовки и разрежте ее на куски удобного размера", 15),
                        CookingStep("Приятного аппетита!", "Разложите по тарелкам с овощами по желанию", 5),
                    ),
                    mutableListOf<Ingredient>(
                        Ingredient("Утка", 1, "шт"),
                        Ingredient("Масло", 50, "г"),
                        Ingredient("Соус", 50, "г")
                    )
                ) {
                    cuisine = "Татарская"
                    complexity = 4
